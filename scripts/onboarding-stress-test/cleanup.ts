...
async function main() {
  console.log('MIA Onboarding Stress Test — Cleanup\n')
  ...
  for (const t of ['lab_sessions', 'learning_events', 'business_memory', 'knowledge_versions', 'knowledge_items', 'sales_rules', 'ai_instructions', 'products', 'assistants', 'brand_identities']) {
  ...
  const dir = __dirname
  const files = fs.readdirSync(dir).filter(f => f.startsWith('.onboarding-') && f.endsWith('.json'))
