# MIA Platform — Technical Reference

This file is the permanent reference for any AI agent working on this repository. It defines the project's purpose, architecture, rules, and quality standards. Every new feature, fix, or refactor must respect these guidelines.
...
5. **Run Chrome DevTools MCP** — check console and network
6. **Fix all errors found** — do not leave known errors unresolved
7. **Verify git status** — `git status` confirms clean working tree after commit
8. **Verify remote sync** — `git push origin main` succeeds
9. **Do not deliver code with known issues**
...
2. CODE:       Write code + tests
3. QUALITY:    npm run lint && npx tsc --noEmit && npm run test:unit && npm run build
4. INTEGRATE:  git add . && git commit -m "feat: ..." && git push origin feat/xxx
5. PR → CI → merge → auto deploy (Vercel)
```

No manual gates. No governance CLI. No Subaru checkpoints. No agent council.

**Quality Gate = lint + typecheck + unit tests + build (automated in CI).**
