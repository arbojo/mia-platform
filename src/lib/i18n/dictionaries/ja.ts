...
export const ja: Dict = {
  ...
  settings: {
    ...
    retentionOffer: 'リテンションオファー',
    retentionDiscountPercent: '提供する割引 (%)',
    retentionDiscountMessage: 'リテンションオファーメッセージ',
    variables: '利用可能な変数',
    preview: 'プレビュー',
    save: '設定を保存',
    saved: '設定が保存されました',
  ...
  ai: {
    ...
    languageMatching:
      '常に顧客と同じ言語で回答します。顧客が英語で書く場合は英語で、スペイン語で書く場合はスペイン語で、ポルトガル語で書く場合はポルトガル語で回答します。一つのメッセージで言語が混在する場合は、そのメッセージで最も多く使われている言語で回答します。どの言語で回答するか尋ねる必要はありません。',
